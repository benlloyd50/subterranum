...
pub fn handle_monster_turns(world: &mut World, map: &mut Map, msg_log: &mut Vec<Message>) {
    for (_, player_pos) in world.query::<With<&Position, &Player>>().iter() {
        ...
        let player_point = map.idx_to_point(player_idx);

        for (_, (mut pos, view, breed)) in world.query::<(&mut Position, &mut ViewShed, &Breed)>().iter() {
            println!("MonIndex: {}", map.xy_to_idx(pos.x, pos.y));
            ...
            if view.visible_tiles.contains(&player_point) {
                ...
                if path.success && path.steps.len() > 1 {
                    ...
                    if try_move(map, next_pos, &mut pos, view) {
                        msg_log.push(Message::new(format!("{}: Im coming, to {:?}", breed.name, next_pos)));
                        view.dirty = true;
                        continue;
                    }
                }
